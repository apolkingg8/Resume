    ...
    $.scrollorama = function(options) {
		...
		function init() {
			...
			for (i=0; i<scrollorama.settings.blocks.length; i++) {
				...
				blocks.push({
					block: block,
					top: block.position().top - (!Boolean(marginTop) ? parseInt(marginTop, 10) : 0),
					pin: 0,
					animations:[]
				});
			}
			...
			latestKnownScrollY = 0;
            ticking = false;
            // $('#container').on( 'scroll.scrollorama', onScroll );
            $('#container').on( 'scroll.scrollorama', onScroll );
		}

		function onScroll() {
            latestKnownScrollY = $('#container').scrollY;
            requestTick();
        }
				...
				if (blocks[i].animations.length) {
					for (j=0; j<blocks[i].animations.length; j++) {
						...
						if (i === currBlockIndex || (currBlockIndex === i-1 && anim.baseline === 'bottom')) {
							...
							if (anim.baseline === 'bottom') { startAnimPos -= $('#container').height(); }
		...
		scrollorama.destroy = function () {
			...
			$('#container').off('scroll.scrollorama');
			$('#scroll-wrap').remove();
